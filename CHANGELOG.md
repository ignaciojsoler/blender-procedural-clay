...
First public release. Beta: tested by the author on Blender 5.2 (Linux, AMD integrated GPU) and in automated headless tests on Blender 5.0. Feedback from Windows, macOS and NVIDIA users is welcome before 1.0.

- UV-free procedural clay material (lumps, pores, grain, specks, color variation) tuned for EEVEE.
- Fingerprints: overlapping finger presses with raised rims and ridges; custom texture support.
- Clay Deform (Geometry Nodes): lumpy silhouette, adaptive subdivision, crease-safe.
- Sticky textures under armatures and shape keys.
- Stop Motion: held poses, surface boil, stepped animation; works when rendered without the add-on.
- Presets, Studio Setup, Draft / Final quality modes.
- Packaged as a Blender extension (4.2+). GPL-3.0-or-later.

## Development log

Internal builds made before the first public release. They were never published; their numbers are kept only so this log matches the commit history.

### Internal build 2.8.0
